"""
Philatelic Pattern Matching and Metadata Enrichment

This module contains all the patterns, regex definitions, and enrichment logic
for philatelic documents extracted from the dolphin_parser.ipynb notebook.
"""
...
def _norm_price(s: str) -> Optional[Dict[str, Any]]:
    ...
    if not raw:
        return None
    num = raw[0]
    # heurística simple; ajústala a tu corpus
    try:
...
def enrich_all_chunks_filatelia(ox: Dict[str, Any]) -> Dict[str, Any]:
    """
    Enriquecimiento en lote para un JSON estilo OXCART:
    agrega metadatos a cada chunk y anota versión.
    """
    for ch in ox.get("chunks", []):
        enrich_chunk_filatelia(ch)
    ox.setdefault("extraction_metadata", {})["enrichment_version"] = "philately-ruleset-1"
